...
}elseif($type=='usdt'){ //USDT
	$DB->exec("UPDATE `pre_pay` SET `type`=:type,channel=:channel WHERE `trade_no`=:trade_no", [':type'=>$type, ':channel'=>'eupay', ':trade_no'=>$orderid]);
	echo "<script>window.location.replace('./eupay.php?trade_no={$orderid}');</script>";
}else{
	exit('该支付方式已关闭');
}
