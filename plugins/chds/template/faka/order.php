...
.payment-icon-usdt {
    ...
    width: 99px;
    height: 30px;
    background-image: url(assets/faka/images/usdtpaym.png);
    background-position: 0px 69px;
}
...
if($config['eupay_api'])echo '<li data-paytype="usdt_pay"><i class="payment-icon-usdt">USDT-TRC20</i></li>';
?>
          </ul>
		  ...
		  <?php if($conf['alipay_api']){?><option value='alipay'>支付宝支付</option><?php }?>
		  <?php if($conf['wxpay_api']){?><option value='wxpay'>微信支付</option><?php }?>
		  <?php if($conf['qqpay_api']){?><option value='qqpay'>QQ钱包支付</option><?php }?>
		  <?php if($config['eupay_api']){?><option value='usdt'>USDT</option><?php }?>		
 			</SELECT>
        </div>
		</div> 
        </div>
